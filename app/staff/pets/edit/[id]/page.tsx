...
export default function EditPetPage() {
  ...
  const fetchPetData = async (petId: number) => {
    try {
      ...
      // Fetch pet data and initial data in parallel
      const [petRes, speciesRes, breedsRes, sheltersRes] = await Promise.all([
        api.getPet(petId),
        fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001'}/species`),
        fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001'}/breeds`),
      ...
      setPet(petRes.pet);
      setFormData({
        name: petRes.pet.name,
        speciesId: petRes.pet.species.id.toString(),
        breedId: petRes.pet.breed.id.toString(),
        shelterId: petRes.pet.shelter.id.toString(),
        age: petRes.pet.age.toString(),
        gender: petRes.pet.gender,
        size: petRes.pet.size,
        color: petRes.pet.color,
        description: petRes.pet.description,
        status: petRes.pet.status,
        isNeutered: petRes.pet.isNeutered,
        isVaccinated: petRes.pet.isVaccinated,
        imageUrl: petRes.pet.imageUrl || '',
      });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001'}/pets/${params.id}`, {
        method: 'PUT',
        headers: {
      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50">
        <div className="container mx-auto px-4 py-8">
          <div className="text-center">
            ...
            <p className="text-gray-600 mb-6">The pet you are looking for does not exist.</p>
            <Button onClick={() => router.push('/staff/pets')} className="bg-blue-600 hover:bg-blue-700">
